...
class ThreeInputGateBlock(
    settings: Settings,
    private val action: (Boolean, Boolean, Boolean) -> Boolean,
    ...
    override fun hasPower(world: World, pos: BlockPos, state: BlockState): Boolean {
        return action(
            hasLeftPower(world, pos, state),
            hasRearPower(world, pos, state),
            hasRightPower(world, pos, state)
        )
    }
}